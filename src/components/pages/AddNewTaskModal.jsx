...
const AddNewTaskModal = ({ open, onClose, onCreate }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-lg bg-white dark:bg-white-500 rounded-2xl shadow-xl border border-white-200 dark:border-white-700 p-5">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded p-2 text-gray-500 hover:text-gray-800 dark:text-red-500 :hover:text-white">
            ❌
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="px-5 py-4 space-y-4">
          {/* Title */}
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-black-300">
              Title
          ...
          <div>
            ...
            <input
              ...
              className="mt-1 w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white 
              dark:bg-gray-800 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500"
            />
            {labelTokens.length > 0 && (
              <div className="mt-2 flex flex-wrap gap-2">
                {labelTokens.map((l, i) => (
                  <span
                    key={`${l}-${i}`}
                    className="text-[10px] font-semibold px-2 py-1 rounded bg-blue-100 
                    text-blue-800 dark:bg-blue-900 dark:text-blue-100"
                  >
                    {l}
                  </span>
                ))}
          ...
          {/*End Date*/}
                <div>
                    <label className='block text-sm font-medium text-gray-700 dark:text-black-300'>End Date
                ...
                <div className='flex items-center justify-end gap-2 pt-1'>
                    <button type="button" onClick={onClose} disabled= {submitting} className='px-4 py-2 text-sm rounded-md border border-gray-300 text-gray-700
                     hover:bg-gray-50 dark:border-gray-700 dark:text-gray-800 dark:hover:bg-gray-600'>Cancel</button>
                    <button 
                    type="submit"
                    disabled= {submitting}
